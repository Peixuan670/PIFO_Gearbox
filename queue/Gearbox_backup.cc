    ...
    if (departureRound / (100*TIMEUNIT) - currentRound / (100*TIMEUNIT) > 1 || insertLevel == 2) {
        ...
        if (!level1InsertingB) {
            ...
            if (departureRound / (10*TIMEUNIT) % 10 == 5) {
                ...
                this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
                decadeLevel.enque(packet, (int)(departureRound / TIMEUNIT)  % 10);
            ...
            if (departureRound / (10*TIMEUNIT) % 10 == 5) {
                currFlow->setInsertLevel(0);
                this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
                decadeLevelB.enque(packet, (int)(departureRound/TIMEUNIT) % 10);
                fprintf(stderr, "Enqueue Level B 1, decede FIFO, fifo %d\n", departureRound  % (10*TIMEUNIT)); // Debug: Peixuan 07072019
            } else {
                currFlow->setInsertLevel(1);
                this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
                levelsB[1].enque(packet, (int)(departureRound/TIMEUNIT) % 10);
        ...
        if (!level0InsertingB) {
            ...
            currFlow->setInsertLevel(0);
            this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
            levels[0].enque(packet, (int)(departureRound/TIMEUNIT) % 10);
            fprintf(stderr, "Enqueue Level 0, regular FIFO, fifo %d\n", departureRound % (10*TIMEUNIT)); // Debug: Peixuan 07072019
        } else {
            //fprintf(stderr, "Enqueue Level B 0\n"); // Debug: Peixuan 07072019
            currFlow->setInsertLevel(0);
            this->updateFlowPtr(iph->saddr(), iph->daddr(),currFlow);  //12182019 Peixuan
            levelsB[0].enque(packet, (int)(departureRound/TIMEUNIT) % 10);
...
vector<Packet*> hierarchicalQueue_pl::serveUpperLevel(int currentRound) {
    ...
    if (currentRound / (100*TIMEUNIT) % 10 == 5) {
        ...
        if (!level1ServingB) {
            size = static_cast<int>(ceil((hundredLevel.getCurrentFifoSize() + levels[1].getCurrentFifoSize()) * 1.0 / (10 - (int)(currentRound/TIMEUNIT) % 10)));  // 07212019 Peixuan *** Fix Level 2 serving order (fixed)
        } else {
            size = static_cast<int>(ceil((hundredLevel.getCurrentFifoSize() + levelsB[1].getCurrentFifoSize()) * 1.0 / (10 - (int)(currentRound/TIMEUNIT) % 10)));  // 07212019 Peixuan *** Fix Level 2 serving order (fixed)
        }
        ...
        if ((int)(currentRound/TIMEUNIT) % 10 == 9)
        ...
        int size = static_cast<int>(ceil(levels[2].getCurrentFifoSize() * 1.0 / (100 - (int)(currentRound/TIMEUNIT) % 100)));
        for (int i = 0; i < size; i++) {
    ...
    if (currentRound / (10*TIMEUNIT) % 10 == 5) {
        ...
        if (!level1ServingB) {
            if (!levels[1].isCurrentFifoEmpty()) {
                int size = static_cast<int>(ceil(levels[1].getCurrentFifoSize() * 1.0 / (10 - (int)(currentRound/TIMEUNIT) % 10)));   // 07212019 Peixuan *** Fix Level 1 serving order (ori)
            ...
            if (!levelsB[1].isCurrentFifoEmpty()) {
                int size = static_cast<int>(ceil(levelsB[1].getCurrentFifoSize() * 1.0 / (10 - (int)(currentRound/TIMEUNIT) % 10)));   // 07212019 Peixuan *** Fix Level 1 serving order (ori)
                //int size = static_cast<int>(ceil((hundredLevel.getCurrentFifoSize() + levels[1].getCurrentFifoSize()) * 1.0 / (10 - currentRound % 10)));  // 07212019 Peixuan *** Fix Level 1 serving order (fixed)
